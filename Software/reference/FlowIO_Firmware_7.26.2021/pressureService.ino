...
void createPressureService(void) {
  ...
  chrPressureValue.setProperties(CHR_PROPS_READ | CHR_PROPS_NOTIFY | CHR_PROPS_WRITE);
  chrPressureValue.setPermission(SECMODE_OPEN, SECMODE_OPEN);
  chrPressureValue.setFixedLen(4);
  chrPressureValue.setWriteCallback(onWrite_chrPressureValue);
...
void onWrite_chrPressureValue(uint16_t conn_hdl, BLECharacteristic* chr, uint8_t* data, uint16_t len) {
